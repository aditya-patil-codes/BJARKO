...
const emptySlot = document.getElementsByClassName(".empty-slot");
...
themeButton.addEventListener("click", () => {
  ...
  const emptySlots = document.getElementsByClassName("empty-slot");
  function randomShade(hue, lightness) {
    return `hsl(${hue}, 60%, ${lightness}%)`;
  }

  const hue = Math.floor(Math.random() * 360);

  const color1 = randomShade(hue, 40);
  const color2 = randomShade(hue, 20);
  const color3 = randomShade(hue, 60);
  body.style.background = `radial-gradient(circle at center, ${color1}, ${color2})`;
  themeButton.style.backgroundColor = color1;
  for (let i = 0; i < emptySlots.length; i++) {
    emptySlots[i].style.color = color3; 
  }
    ...
    for (let i = 0; i < players.length; i++) {
      if (players[i].hasLeft === true) {
        if (players[i].clientId === playerSlotHTML[x]) {
          ...
          <div class="empty-slot noclick">+</div>
...
$("#invite-link-box button").click(async function () {
  try {
...
function setTimer(duration) {
  ...
  let timeUntilDeal = setInterval(function () {
    ...
    if (players.every((player) => player.isReady === false)) {
      clearInterval(timeUntilDeal);
      clearInterval(timeUntilDealExtra);
      document.getElementById("milliseconds").innerHTML = 0; // stop on 0
      $("#deal-start-label").addClass("hide-element");
    }
  }, 1000);
